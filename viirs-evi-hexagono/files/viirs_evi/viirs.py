...
def latest_composite_date(
    ...
    """
    Última data de composição VNP13A1 disponível no CMR para o balde
    ``tiles_hv`` dentro de ``[start, end]`` (strings YYYY-MM-DD).

    Usada pelo processamento incremental para decidir até onde há dado novo,
    **sem** baixar nada — só a busca no CMR (anônima).
    """
    bbox = template.rio.bounds()
    grans = _with_retries(
        search_granules_bbox, bbox, start, end,
        what=f"search latest {bucket_label_safe(tiles_hv)}",
    )
    tiles_set = set(tiles_hv)
    dates = [granule_date(g) for g in grans if _safe_tile(g) in tiles_set]
    return max(dates) if dates else None


def bucket_label_safe(tiles_hv) -> str:
...
def build_tile_cube(
    ...
    """Compat.: cubo do ano-calendário ``year`` (delega para o range)."""
    return build_tile_cube_range(
        tiles_hv, f"{year}-01-01", f"{year}-12-31", template, tmp_dir, label
    )


def build_tile_cube_range(
    tiles_hv: list[tuple[int, int]],
    start: str,
    end: str,
    template: xr.DataArray,
    ...
    o balde de tiles ``tiles_hv``) para a janela ``[start, end]`` (strings
    YYYY-MM-DD).
    ...
    (``VIIRS_PREFETCH_DATES``), então não seguramos toda a janela de granules
