...
def fit_curve_to_cycle(ndvi_values: np.ndarray, dates: np.ndarray, cycle: Dict[str, Any],
    ...
    # k_min: transição 10%→90% em no máximo ln(81)/k_min dias; 0.02 → ~220 dias (cana lenta)
    # k_max: transição 10%→90% em no mínimo ln(81)/k_max dias; 0.5  →   ~9 dias (ciclo rápido)
    _k_min, _k_max = 0.02, 0.5
    lower_bounds = [0.01, -slack,        _k_min,  peak_pos_days, _k_min,  series_min]
...
def _bank_quality_ok(c: Dict, min_r2: float = 0.70) -> bool:
    ...
    if not (0.02 < k2  < 0.5):  return False
    if not (0.02 < k1  < 0.5):  return False
